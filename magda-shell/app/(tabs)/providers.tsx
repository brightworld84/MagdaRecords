...
import { View, Text, TextInput, TouchableOpacity, FlatList, Modal, StyleSheet, Platform, KeyboardAvoidingView } from 'react-native';
...
export default function Providers() {
  ...
  return (
    <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : undefined} style={styles.container}>
      <ScrollView>
        <Text accessibilityRole="header" style={styles.header}>My Providers</Text>
          ...
          <ScrollView contentContainerStyle={styles.modalContent}>
            {['name', 'specialty', 'address', 'phone', 'email', 'fax', 'notes'].map(field => (
              <View key={field} style={styles.inputGroup}>
                <Text style={styles.label}>{field === 'name' ? '🏥 Name or Facility' :
...
const styles = StyleSheet.create({
  ...
  addButton: { backgroundColor: '#007AFF', padding: 12, borderRadius: 8, marginBottom: 20 },
  addButtonText: { color: 'white', textAlign: 'center', fontSize: 16 },
  card: { padding: 16, backgroundColor: '#f2f2f2', borderRadius: 8, marginBottom: 10 },
  ...
  saveButton: { backgroundColor: '#34C759', padding: 12, borderRadius: 8, marginTop: 10 },
  saveButtonText: { color: 'white', textAlign: 'center', fontSize: 16 },
  cancelButton: { backgroundColor: '#ff3b30', padding: 12, borderRadius: 8, marginTop: 10 },
